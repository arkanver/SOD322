...
void prudentclean(char* input, char* output){
  ...
  edge *edges = malloc(NLINKS*sizeof(edge));
  unsigned long n = 0;
  ...
  while (fscanf(file,"%lu %lu", &a, &b)==2){
    ...
    if (a != b){
      while (!found && i < n){
        if (edges[i].s == a && edges[i].t == b){
          found = true;
        }
        else{
          if (edges[i].s == b && edges[i].t == a){
            found = true;
          }
        }
        i++;
